...
{
  tc<-round((4*sqrt(descr[,2])+1.5*descr[,3])/(0.8*sqrt(descr[,4]-descr[,5])),digits=2)#Giandotti, 1934
  names(tc)<-descr[,1]
  return(tc)
}
...
exponential_model_2<-function(pars)
{
  λ1<-pars[1]
  λ2<-pars[2]
  est<-.5*exp(-λ1*norm_durations/(1+λ2*norm_durations))
  sumsq<-sum((est-obs)^2)
  return(sumsq)
}
...
{
  τ<-pars[1]
  est<-τ/norm_durations*(1-exp(-norm_durations/τ))
  sumsq<-sum((est-obs)^2)
  return(sumsq)
}
...
#' @export
rD_εD_D_pars<-function(streamflows_HH,Df_target,d_threshold,basin_descriptors)
{
  if(length(which(is.na(streamflows_HH[,3])))>0) streamflows_HH<-streamflows_HH[-which(is.na(streamflows_HH[,3])),]#NA removal
  
  stations<-unique(streamflows_HH[,1])
  
  durations<-c(1:(Df_target+1))
  ...
  for(r in stations)
  {
    str_r_HH<-streamflows_HH[which(streamflows_HH[,1]%in%r),-1]
    
    str_r_GG <- str_r_HH %>% dplyr::group_by(Datetime=as.Date(Datetime)) %>% dplyr::summarize(Value = mean(Value,na.rm=TRUE)) %>% as.data.frame()
    
    #PORFDC
    Q=unique(sort(str_r_GG[,2],decreasing = TRUE))
    ...
    for(j in durations)
    {
      vol_r_HH <- cbind.data.frame(Datetime=str_r_HH[,1],Value=moving_average(str_r_HH[,2]))
      ...
      for(e in 1:length(str_GG_d[,1]))
      {
        ...
        index_str <- which(substring(str_r_HH[,1],1,10)%in%c(str_GG_d[e,1]-1,str_GG_d[e,1],str_GG_d[e,1]+1))
        
        t_start <- vol_r_HH[index_vol,1][which.max(vol_r_HH[index_vol,2])]
        t_peak  <- str_r_HH[index_str,1][which.max(str_r_HH[index_str,2])]
  ...
  rD_D_pars<-data.frame(matrix(NA,length(rD_D_mean[,1]),3,dimnames=list(x=NULL,y=c("Name","λ1","λ2"))))
  ...
  εD_D_par<-data.frame(matrix(NA,12,2,dimnames=list(x=NULL,y=c("Name","τ"))))
  ...
  τ_Tc_pars <- data.frame(matrix(NA,nrow=1,ncol=4,dimnames = list(x=NULL,y=c("Parameter","Formula","q","m"))))
  τ_Tc_pars[,1] <- colnames(εD_D_par)[-1]
  x<-as.numeric(ToC)
  
  τ_Tc_pars[,3:4]<-linear_model(εD_D_par[,2])$coefficients
  τ_Tc_pars[,2]<-"y=q+m*x"
  
  #Store functional relationships of reduction ratio and peak-duration curve's parameters inside a list
  rD_εD_D_pars <- list(λs_Tc_pars=λs_Tc_pars,τ_Tc_pars=τ_Tc_pars)
  return(rD_εD_D_pars)
}
