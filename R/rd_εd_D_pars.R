...
rd_εd_D_pars<-function(streamflows_HH,Df_target,d_threshold)
{
  ...
  Qd_ds<-list()#maximum average discharge in a given duration for each flood event
  rd_ds<-list()#time-to-peak for a given duration for each flood event
  ...
  for(r in stations)
  {
    ...
    {
      if(str_GG_d[i,1]-str_GG_d[i-1,1]==1 
         && str_GG_d[i+1,1]-str_GG_d[i,1]==1)
      {
        if(str_GG_d[i,2]>str_GG_d[i-1,2] && 
           str_GG_d[i,2]>str_GG_d[i+1,2] && 
           str_GG_d[i,3]<=d_threshold) index<-append(index,i)
      }
    }
    index<-unique(index)
    
    str_GG_d<-str_GG_d[index,]
    ...
    for(j in durations)
    {
      ...
      Res_Media_Mobile <- cbind.data.frame(DATAORA=str_r_HH$DATAORA,VALORE=Media_Mobile(str_r_HH$VALORE))
      ...
      { 
        ...
        Index_HH <- which(substring(str_r_HH$DATAORA,1,10)%in%c(Qgg_d$DATA[d]-1,Qgg_d$DATA[d],Qgg_d$DATA[d]+1))
        t_start <- Res_Media_Mobile$DATAORA[Index_MM][which.max(Res_Media_Mobile$VALORE[Index_MM])]
        t_peak  <- str_r_HH$DATAORA[Index_HH][which.max(str_r_HH$VALORE[Index_HH])]
...
for(fn in file_names)
{
  ...
  str_r_GG <- Q_hh %>% group_by(DATA) %>% dplyr::summarize(VALORE = mean(VALORE, na.rm=TRUE)) %>% as.data.frame()
