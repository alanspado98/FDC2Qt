...
#Exponential model function: y=a+b*exp(-c*x) for HH/GG = f(duration) fitting
...
#' @export
peak_ratio_parameters<-function(streamflows_HH,basin_descriptors,target_section,d_threshold)
{
  #Observed hourly streamflow series
  if(length(which(is.na(streamflows_HH[,3])))>0) streamflows_HH<-streamflows_HH[-which(is.na(streamflows_HH[,3])),]
  ...
  for (r in unique(streamflows_HH[,1]))
  {
    ...
    #Corresponding daily streamflows
    `%>%` <- magrittr::`%>%`
    ...
    max_HH_daily<-streamflows_HH_r %>% #maximum hourly streamflow
      ...
      dplyr::summarise(Value = max(Value,na.rm=T),.groups="drop") %>%
      as.data.frame()
  ...
  #Exponential models function for a=f(Tc), b=f(Tc), c=f(Tc) (no constraint point to satisfy)
