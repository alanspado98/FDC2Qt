...
hourly_streamflows<-function(reg_daily_str,target_section,abc_Tc_pars,τ_Tc_pars,λs_Tc_pars,Df_target,d_threshold)
{
  ...
  w_Q <- .5 #weight of streamflows mean relative error (Synthetic Design Hydrograph constraint)
  Delta_threshold<-.25 #metric threshold (weighted average of relative errors)
  max_iter <- 20 #number of iteration threshold
  ...
  for(l in 1:length(Group_cons_years))#cycle on each group of consecutive years
  {
    ...
    index_flood<-c()
    for(i in 2:(length(str_GG_l[,1])-1))
    {
      if(str_GG_l[i,2]>=str_GG_l[i-1,2] && 
         str_GG_l[i,2]>=str_GG_l[i+1,2] && 
         str_GG_l[i,3]<=d_threshold) index_flood<-append(index_flood,i)
    }
