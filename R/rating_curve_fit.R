...
#'   observation (numeric)
#'
#' @return `rating_curves` dataframe
#'
#' @details 
...
#'
#' @export
load("data/availability_matrices.rda")
fit_rating_curve<-function(stream_stages,rating_curves,hl_DA_matrix,rc_DA_matrix,str_DA_matrix)
{
  ...
  for(r in stations)
  {
    ...
    hl_r<- hl_r %>%#Daily hydrometric levels
      ...
      as.data.frame() 
    str_r<-streamflows[streamflows[,1]%in%r,-1]
  ...
  return(rating_curves)#It should be revaluated only for rating curves
}